...
<title>User Profile</title>
	...
	<%	
		...
		following = (ArrayList<String>)request.getSession().getAttribute("userfing");
		follower = (ArrayList<String>)request.getSession().getAttribute("userfer");
		read = (ArrayList<String>)request.getSession().getAttribute("userread");
		fav = (ArrayList<String>)request.getSession().getAttribute("userfav");
		String userurl = (String)request.getSession().getAttribute("user_url");
		String username = (String)request.getSession().getAttribute("username");
		ArrayList<User> userList = (ArrayList<User>)request.getSession().getAttribute("userlist");
		String follow = (String)request.getSession().getAttribute("follow");
		/* if(follow == null){ */
			follow = "Follow";
			for(int i =0; i <userList.size();i++){
				if(name.equals(userList.get(i).getName())){
					for(int j = 0;j < userList.get(i).getFollowing().size();j++){
						if(username.equals(userList.get(i).getFollowing().get(j))){
							follow = "Unfollow";
						}
					}
				}
	...
	<body onload = "userIconReset();">
		<div class = "container2">
			...
			<div id = "result"> 
					...
					<div class = "lib_top">
						<div class = "lib"><%=username%>'s library</div>
						<a onclick= "follow('<%=username%>','<%=name%>')"><div id = "followed" class = "back"><%=follow %></div></a>
						
					</div>
					
						<%-- <a onclick = "getBookimg('<%=readString%>');"><div id = "rop" class = "read_op">Read</div></a> --%>
						
						<a onclick = "getfavBookimg2('<%=favString%>');"><div id = "fop" class = "fav_op">Favorites</div></a>
